...
for (const tier of tiers) {
	...
	const outFile = path.join(OUT_DIR, `${tier.name}.woff2`);
	...
	faces.push({
		file: `${tier.name}.woff2`,
		size: fs.statSync(outFile).size,
		range: toUnicodeRange(tier.chars),
	});
}

// 文件名不带内容哈希，所以 public/_headers 与 vercel.json 里给 result.css
// 配的是 must-revalidate 而不是 immutable
...
console.log(
	`\nDone (source ${(srcSize / 1024).toFixed(0)}KB):\n` +
		faces
			.map(
				(f) =>
					`  ${f.file.padEnd(12)} ${(f.size / 1024).toFixed(0).padStart(4)} KB`,
			)
			.join("\n") +
		`\n  ${"result.css".padEnd(12)} ${(fs.statSync(CSS_FILE).size / 1024).toFixed(0).padStart(4)} KB` +
