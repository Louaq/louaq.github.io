 ...
 *
 * 输入是 subset-font 的产物（19MB 原始 TTF 没进仓库）。
 */
...
const FONT_FAMILY = "LXGWWenKai_Regular";
...
${faces
	.map(
		(face) =>
			`@font-face{font-family:"${FONT_FAMILY}";src:local("LXGW WenKai"),local("LXGWWenKai"),url("./${face.file}")format("woff2");font-style:normal;font-weight:400;font-display:swap;unicode-range:${face.range}}`,
	)
	.join("\n")}
`;
